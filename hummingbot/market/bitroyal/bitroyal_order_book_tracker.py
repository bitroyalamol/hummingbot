...
class BitroyalOrderBookTracker(OrderBookTracker):
        ...
        *required
        ...
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = BitroyalAPIOrderBookDataSource(trading_pairs=self._trading_pairs)
            else:
    ...
    async def _refresh_tracking_tasks(self):
        ...
        for trading_pair in new_trading_pairs:
            order_book_tracker_entry: BitroyalOrderBookTrackerEntry = available_pairs[trading_pair]
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ...
                order_book: BitroyalOrderBook = self._order_books[trading_pair]
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: BitroyalOrderBook = self._order_books[trading_pair]
        active_order_tracker: BitroyalActiveOrderTracker = self._active_order_trackers[trading_pair]
        ...
        while True:
            try:
                ...
                saved_messages: Deque[BitroyalOrderBookMessage] = self._saved_message_queues[trading_pair]
